...
export function Matchmaking() {
  ...
  const loadSessionChats = useCallback(
    async () => {
      try {
  ...
  }, [socket, isConnected, currentUserId, subscribe, unsubscribe, transitionToSession, stopPolling]);
          ...
          <div className="matchmaking-matched">
            ...
            {sessionData?.id && (
              <p className="session-meta">
                Session: <span className="session-id">{sessionData.id}</span>
              </p>
            )}
            {otherUserName && (
              <p>
                You will be chatting with <strong>{otherUserName}</strong>
              </p>
            )}
        ...
        return (
          <div className="chat-session">
            <div className="chat-header">
              <div>
                <h2>
                  {otherUserName ? `Chat with ${otherUserName}` : 'Chat Session'}
                </h2>
                <div className="session-meta">
                  <span className="role-badge">{role}</span>
                  {sessionData?.id && (
                    <span className="session-id">Session: {sessionData.id}</span>
                  )}
                </div>
              </div>
              ...
              <div className="button-group flex gap-2">
                ...
                <button onClick={leaveSession} className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 rounded">
                  Skip
                </button>
                <button onClick={exitMatchmaking} className="px-4 py-2 bg-red-500 hover:bg-red-600 rounded">
                  Exit Matchmaking
                </button>
              </div>
            </div>
              ...
              {chatMessages.length === 0 ? (
                ...
                chatMessages.map((msg, idx) => {
                  ...
                  return (
                    <div
                      key={key}
                      className={`message ${isOwn ? 'own ml-auto' : 'other mr-auto'} max-w-xs`}
                    >
                      <div className={`message-content ${isOwn ? 'bg-primary text-dark' : 'bg-dark text-primary'} rounded-lg p-3`}>
            ...
            {error && <div className="error text-red-500 p-3 text-center">{error}</div>}
          </div>
        );
