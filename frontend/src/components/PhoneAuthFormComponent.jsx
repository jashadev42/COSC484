...
export default function PhoneAuthFormComponent() {
    ...
    return (
        <div className="min-h-screen w-full flex items-center justify-center p-6 text-white">
            ...
            {!otpSent && !session && (
                <form className="space-y-4" onChange={() => setError("")} onSubmit={onGetOtp}>
                    ...
                    <input
                        name="phoneNumber"
                        type="tel"
                        className="w-full rounded-2xl px-6 py-4 bg-white text-neutral-200 font-semibold transition-colors shadow-md"
            ...
            {/* input for OTP */}
            {otpSent && !session && (
                <form className="space-y-4 text-center flex flex-col items-center" onSubmit={onVerify}>
                    {error && <p className="text-red-400">{error}</p>}
                    <p className= "text-green-300 max-w-80"> code sent! your phone should receive an 6 digit code shortly. </p>
                    <input
                        inputMode="numeric"
                        pattern="[0-9]*"
                        maxLength={6}
                        className="w-full rounded-2xl px-6 py-4 bg-white font-semibold text-netural-200 transition-colors shadow-md"
                        value={code}
                        // only allow numbers
                        onChange={(e) => {
